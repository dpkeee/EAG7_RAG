...
def search_documents(query: str) -> list[str]:
    ...
    try:
        mcp_log("DEBUG", f"Loading index from {index_path}")
        index = faiss.read_index(str(index_path))
        mcp_log("DEBUG", f"Index loaded with {index.ntotal} vectors")
        
        with open(meta_path, "r") as f:
            metadata = json.load(f)
        mcp_log("DEBUG", f"Loaded {len(metadata)} metadata entries")
        
        query_vec = get_embedding(query).reshape(1, -1)
        mcp_log("DEBUG", f"Query vector shape: {query_vec.shape}")
        
        D, I = index.search(query_vec, k=1)
        mcp_log("DEBUG", f"Search results - distances: {D}, indices: {I}")
...
def ensure_faiss_ready():
    ...
    mcp_log("DEBUG", f"Looking for index at: {index_path}")
    mcp_log("DEBUG", f"Looking for metadata at: {meta_path}")
...
def save_faiss_index(metadata_list=None):
    ...
    try:
        ...
        # Save metadata (as a list of dicts: doc, chunk, chunk_id)
        with open(FAISS_MAPPING_FILE, "w") as f:
            json.dump(metadata_list if metadata_list else [], f, indent=4)
